...
Creative 아이디어 보드 워크플로우 ExecPlan 생성 완료, 구현 승인 대기

이 저장소는 Unity 프로젝트와 최소 Codex 하네스를 준비했고, creative/art 워크플로우 문서 구조와 엄격 모드 라우팅 규칙을 적용했습니다. 현재는 기존 creative/art 분류가 사용자가 원한 가벼운 브레인스토밍보다 무거운 게이트처럼 작동한다는 문제를 확인했고, `creative`를 여러 게임 아이디어를 카드처럼 꺼내고 고르는 아이디어 보드로 재정의하기 위한 ExecPlan을 작성했습니다. 아직 player-facing gameplay design은 승인되지 않았고, gameplay 구현도 시작하지 않았습니다.

## 활성 계획

`exec-plans/003-creative-idea-board-workflow.md`

이 계획은 creative/art/design 분리 원칙은 유지하되, creative를 승인 게이트가 아니라 여러 게임 아이디어를 자유롭게 만들고 고르는 보드형 브레인스토밍 흐름으로 재정의하는 계획입니다. 현재 상태는 `구현 승인 대기`입니다.
...
- 현재 creative/art 분류가 사용자가 원한 아이디어 발산 흐름보다 무거운 게이트처럼 작동한다는 문제를 사용자와 논의했습니다.
- creative 첫 단계는 한 게임의 구체화가 아니라 여러 게임 아이디어를 자유롭게 뽑고 고르는 보드라는 합의를 확인했습니다.
- creative 단계에서 조작, 규칙, 성공/실패, 화면 아이디어도 러프한 후보 메모로 허용한다는 합의를 확인했습니다.
- creative 산출물은 보드형으로 운영하고, 초기에는 점수표나 우선순위 평가표를 두지 않는다는 합의를 확인했습니다.
- art는 creative 카드 안에 `아트/분위기 상상`만 가볍게 두고, 후보 선택 후 별도 art 문서로 확장한다는 합의를 확인했습니다.
- `exec-plans/003-creative-idea-board-workflow.md`를 작성하고 구현 승인 대기 상태로 두었습니다.

## 다음 단계

1. 사용자에게 `exec-plans/003-creative-idea-board-workflow.md` 구현 승인 여부를 확인합니다.
2. 구현 승인을 받으면 `docs/creative/README.md`, `AGENTS.md`, `docs/art/README.md`를 아이디어 보드 흐름에 맞게 갱신합니다.
3. 기존 `docs/creative` 문서가 새 보드형 흐름과 충돌하지 않게 정리합니다.
...
- Creative 산출물은 행동 횟수, 상태명, UI 조작, 성공/실패 조건 같은 gameplay 요소를 러프한 후보 메모로 적을 수 있지만, design 승인 전에는 확정 사양이나 구현 기준이 아닙니다.

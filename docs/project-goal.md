...
- 복구 대상 오브젝트가 존재합니다.
- Player가 복구 대상 근처에서 `E` 입력으로 상호작용할 수 있습니다.
...
M1.5 확정값을 따르고, 가장 단순한 거리 조건 방식으로 구현해줘.
...
2. 세 복구 대상의 복구 상태를 관리합니다.
3. 세 복구 대상이 모두 처리되면 완료 상태를 표시합니다.

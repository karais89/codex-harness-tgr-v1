...
- Unity 작업 검증에는 `CoderGamester/mcp-unity` 패키지의 `com.gamelovers.mcp-unity`를 사용합니다.
- Unity MCP의 첫 검증은 `localhost:8090` 연결, Console log 조회, package / asset 리소스 조회, log 왕복 전송처럼 파괴적이지 않은 smoke test부터 진행합니다.
- Scene / GameObject / Play Mode 확인이 MCP 도구만으로 부족하면 Unity Editor 확인으로 보완하고, MCP로 확인하지 못한 범위는 결과에 명시합니다.
...
- Unity 프로젝트가 열립니다.
- `com.gamelovers.mcp-unity` 패키지가 resolve되고 compile error가 없습니다.
- Unity MCP 서버가 `localhost:8090`에서 실행됩니다.
- Codex project-local `.codex/config.toml` 설정으로 MCP 서버를 인식할 수 있습니다.
- Console log 조회와 `mcp smoke test` log 왕복을 확인합니다.
- Package / asset 리소스를 조회할 수 있습니다.
- 현재 Scene 정보와 Play Mode 실행 가능 여부를 MCP 또는 Unity Editor로 확인합니다.

## 예상 작업 흐름

1. Unity 프로젝트를 엽니다.
2. Package Manager resolve와 Console compile error 여부를 확인합니다.
3. `Tools > MCP Unity > Server Window`에서 서버를 `localhost:8090`으로 시작합니다.
4. Codex project-local `.codex/config.toml` 설정을 확인합니다.
5. Console log, package, asset 조회성 smoke test를 실행합니다.
6. `send_console_log`로 `mcp smoke test`를 보낸 뒤 다시 Console log에서 확인합니다.
7. 현재 Scene 정보와 Play Mode 실행 가능 여부를 MCP 또는 Unity Editor로 확인합니다.

## 예상 프롬프트

```text
Unity MCP 연결 상태를 확인해줘.
CoderGamester MCP 기준으로 Console log, packages/assets, 현재 Scene, Play Mode 실행 가능 여부를 점검해줘.
수정하지 말고 확인 결과만 보고해줘.
```
...
- `잡초`, `시든 꽃`, `더러운 타일`이 놓일 수 있는 구역이 있습니다.
- 카메라 이동 없이 정원 전체가 한 화면에 보이는 compact square layout입니다.
- 오브젝트 이름이 명확합니다.
...
4. `잡초`, `시든 꽃`, `더러운 타일`이 놓일 수 있는 구역을 남깁니다.
5. 카메라가 이동 없이 정원 전체를 볼 수 있게 조정합니다.
6. Unity에서 변경 결과를 확인합니다.

## 예상 프롬프트

```text
Tiny Garden Restore의 기본 정원 Scene을 만들어줘.
Player, 작은 정원 영역, 잡초/시든 꽃/더러운 타일 배치 구역만 가장 단순하게 배치해줘.
카메라 이동 없이 한 화면에 전체 정원이 보이게 해줘.
...
복구 대상은 M1.5에서 확정한 `잡초`, `시든 꽃`, `더러운 타일` 중 하나로 시작합니다.

구체 표현은 M1.5 확정값을 따르되, 구현 방식만 가장 단순하게 정합니다.

## 완료 기준

- 복구 대상 오브젝트가 존재합니다.
- Player가 복구 대상 근처에서 `E` 1회 입력으로 상호작용할 수 있습니다.
- 상호작용 후 복구 대상의 상태가 즉시 바뀝니다.
...
1. `잡초`, `시든 꽃`, `더러운 타일` 중 하나의 복구 대상 오브젝트를 생성합니다.
2. Player와 복구 대상의 근처 거리 조건을 구현합니다.
3. `E` 1회 입력 즉시 복구 상호작용을 구현합니다.
...
```text
Player가 정원 안의 복구 대상 근처에서 E를 누르면 상태가 바뀌게 해줘.
M1.5 확정값을 따르고, E 1회 입력 즉시 복구와 가장 단순한 거리 조건 방식으로 구현해줘.
...
2. Player가 이동합니다.
3. Player가 `잡초`, `시든 꽃`, `더러운 타일`과 상호작용합니다.
4. 각 복구 대상의 상태가 복구됨으로 바뀝니다.
5. 세 복구 대상이 모두 처리되면 `Garden Restored` 완료 메시지가 표시됩니다.

## 완료 기준

- 시작 상태가 있습니다.
- 플레이어 행동이 있습니다.
- `잡초`, `시든 꽃`, `더러운 타일` 복구 대상이 있습니다.
- 세 복구 대상은 시작 시점부터 모두 Scene에 보입니다.
- 세 복구 대상 완료 조건이 있습니다.
- `Garden Restored` 완료 결과가 표시됩니다.
- Unity Play Mode에서 처음부터 끝까지 확인 가능합니다.
- Console error가 없습니다.

## 예상 작업 흐름

1. `잡초`, `시든 꽃`, `더러운 타일` 복구 대상 오브젝트를 배치합니다.
2. 세 복구 대상이 시작 시점부터 모두 보이는지 확인합니다.
3. 세 복구 대상의 복구 상태를 관리합니다.
4. 세 복구 대상이 모두 처리되면 `Garden Restored` 완료 상태를 표시합니다.
5. Play Mode에서 전체 루프를 확인합니다.
6. Console error를 확인합니다.
...
완료 메시지는 Garden Restored 단순 텍스트로 해줘.

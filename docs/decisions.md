...
결정:
`docs/creative`는 design 이전의 가벼운 컨셉 탐색 작업장으로 사용한다. 이곳에서는 정서, 감각, 소재, 재미 후보, 피하고 싶은 방향을 다룬다. `docs/art`는 아트 방향, 이미지 프롬프트, 생성 이미지 기록을 위한 공간으로 사용한다. `docs/design`은 기존처럼 구현 기준으로 삼을 수 있는 승인된 player-facing design 사양을 보관한다.

근거:
기획과 아트 탐색 결과를 곧바로 구현 사양으로 취급하면 Codex가 아이디어, 레퍼런스, 승인된 규칙을 섞을 수 있다. 세 폴더의 역할을 분리해야 실제 협업처럼 발산, 시각 탐색, 사양 확정, 구현 계획을 반복할 수 있다.

### creative 단계는 gameplay 세부 사양을 확정하지 않는다

결정:
Creative 단계에서는 행동 횟수, 자원량, 성공/실패 조건, UI 조작, 상태명, 맵 구조, 레벨 구조, 구체 피드백, Unity 구현 방식을 확정하지 않는다. 이런 항목은 player-facing gameplay 기준이므로 필요하면 `docs/design` 절차로 넘긴다.

근거:
컨셉 브레인스토밍이 수치와 조작과 상태명으로 곧바로 수렴하면 사용자가 원한 정서, 감각, 재미 방향 탐색이 사라지고, 승인되지 않은 design spec이 creative 문서에 섞인다.

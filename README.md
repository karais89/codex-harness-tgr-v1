...
- 정원에는 복구해야 할 대상이 몇 개 존재합니다.
- 플레이어는 대상과 상호작용해 정원을 조금씩 복구합니다.
- 모든 필수 복구 대상을 처리하면 하루 또는 스테이지가 완료됩니다.
- 게임은 작고 명확해야 하며, 처음부터 복잡한 성장/경제/스테이지 시스템을 만들지 않습니다.

## 1차 playable loop 예시

구체 규칙은 구현 과정에서 가장 단순하게 정합니다.

예상 loop는 다음과 같습니다.

1. 플레이어가 작은 정원 Scene에서 시작합니다.
2. 플레이어가 이동합니다.
3. 플레이어가 복구 대상과 상호작용합니다.
4. 복구 대상의 상태가 바뀝니다.
5. 모든 필수 복구 대상이 처리되면 완료 메시지가 표시됩니다.

이 루프는 예시이며, 구현 중 더 단순한 방식이 적절하면 작고 되돌리기 쉬운 가정을 명시하고 진행합니다.

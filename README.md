...
이 저장소의 중심 산출물은 큰 게임이나 완성 콘텐츠가 아니라, Codex가 문서 읽기, 사용자 인터뷰, 기획 논의, 아트 방향 탐색, ExecPlan 작성, 구현 승인, Unity 구현, 수동/자동 검증, 다음 세션 인수인계를 반복 가능한 흐름으로 수행할 수 있는지 확인하는 기록입니다.

TGR v1은 `creative`, `art`, `design`, `ExecPlan`, `implementation`이 항상 한 방향으로만 진행된다고 가정하지 않습니다. 개발 중에도 기획 아이디어나 아트 방향이 다시 열릴 수 있으며, Codex가 이 흐름을 섞지 않고 문서화하고 필요한 승인 지점에서 멈출 수 있는지 검증합니다.
...
- `docs/creative/`: 아이디어 발산, 후보 압축, 기획 초안 도출을 위한 작업장입니다.
- `docs/art/`: 아트 방향, 이미지 프롬프트, 생성 이미지 기록을 위한 공간입니다.
